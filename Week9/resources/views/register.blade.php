...
<head>
  ...
  <title>Daftar — Risol Majesty</title>
  ...
  <style>
    ...
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'DM Sans',sans-serif;font-weight:300;min-height:100vh;display:flex;align-items:center;justify-content:center;background-color:#0D0805;padding:24px 0}
    ...
    .register-wrap{position:relative;z-index:1;width:100%;max-width:440px;padding:16px;animation:muncul 0.5s ease both}
    @keyframes muncul{from{opacity:0;transform:translateY(28px)}to{opacity:1;transform:translateY(0)}}
    .register-card{background:rgba(24,13,6,0.95);border:1px solid var(--border);border-radius:22px;padding:40px 36px 34px;box-shadow:0 24px 60px rgba(0,0,0,0.55)}
    ...
    .reg-judul{font-family:'Playfair Display',serif;font-size:1.8rem;font-weight:700;color:var(--cream);line-height:1.2;margin-bottom:4px}
    .reg-judul em{font-style:italic;color:var(--gold)}
    .reg-sub{font-size:0.82rem;color:var(--muted);margin-bottom:26px}
    ...
    .form-control.is-invalid{border-color:rgba(180,45,40,0.6)}
    .invalid-feedback{color:#ECA090;font-size:0.78rem;margin-top:5px}
    .btn-register{width:100%;background:linear-gradient(135deg,var(--amber),var(--gold));border:none;color:#FAF6F1;font-family:'DM Sans',sans-serif;font-size:0.85rem;font-weight:500;padding:12px 20px;border-radius:99px;cursor:pointer;transition:all 0.25s;margin-top:8px}
    .btn-register:hover{background:linear-gradient(135deg,var(--brown),var(--amber));transform:translateY(-2px)}
    .link-kembali{color:var(--muted);font-size:0.83rem;text-decoration:none}
    .link-kembali:hover{color:var(--cream)}
    .alert-success{background:rgba(34,120,60,0.12);border:1px solid rgba(34,120,60,0.3);border-radius:10px;color:#90ECA8;font-size:0.83rem;padding:10px 14px;margin-bottom:18px;display:flex;align-items:center;gap:8px}
...
<script src="https://www.google.com/recaptcha/api.js" async defer></script>
  ...
  <div class="register-wrap">
    <div class="register-card">
      ...
      <h1 class="reg-judul">Buat <em>Akun Baru</em></h1>
      <p class="reg-sub">Isi formulir di bawah untuk mendaftar</p>
      ...
      <div style="display:flex; align-items:center; margin: 20px 0; gap:12px;">
        ...
        <span style="color:var(--muted); font-size:0.8rem;">atau daftar dengan</span>
        <div style="flex:1; height:1px; background:rgba(200,150,100,0.2);"></div>
      </div>
      ...
      <a href="{{ route('google.redirect') }}"
         style="display:flex; align-items:center; justify-content:center; gap:10px;
                width:100%; padding:11px 20px; border-radius:99px;
                background:rgba(255,255,255,0.06); border:1px solid rgba(200,150,100,0.25);
      ...
      <div class="text-center mt-3" style="display:flex; justify-content:space-between; align-items:center; margin-top:16px;">
        <a href="{{ route('home') }}" class="link-kembali"><i class="bi bi-arrow-left me-1"></i>Beranda</a>
        <a href="{{ route('login') }}" class="link-kembali">Sudah punya akun? <span style="color:var(--gold);">Login</span></a>
      </div>
    </div>
  </div>
</body>
