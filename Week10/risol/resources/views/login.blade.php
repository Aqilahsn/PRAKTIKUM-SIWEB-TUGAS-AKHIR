...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .login-wrap{position:relative;z-index:1;width:100%;max-width:420px;padding:16px;animation:muncul 0.5s ease both}
    ...
    .alert-gagal{background:rgba(180,45,40,0.12);border:1px solid rgba(180,45,40,0.3);border-radius:10px;color:#ECA090;font-size:0.83rem;padding:10px 14px;margin-bottom:18px;display:none;align-items:center;gap:8px}
    ...
    .form-check-input{width:16px;height:16px;accent-color:var(--amber);cursor:pointer}
    .form-check-label{font-size:0.83rem;color:var(--muted);cursor:pointer;margin:0}
...
<script src="https://www.google.com/recaptcha/api.js" async defer></script>
  ...
  <div class="login-wrap">
    <div class="login-card">
      ...
      @if(session('error'))
      <div class="alert-gagal" style="display: flex;">
      ...
      <form method="POST" action="{{ route('login.post') }}">
        ...
        <div class="mb-3">
          ...
          <input type="text" name="username" class="form-control" placeholder="admin atau email@gmail.com" required value="{{ old('username') }}">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          @error('g-recaptcha-response')
            <small style="color:#ECA090; font-size:0.8rem;">{{ $message }}</small>
          @enderror
        </div>
